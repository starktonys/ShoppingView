...
public class ShoppingView extends View {
  ...
  private final static int DEFAULT_DURATION = 0;
  ...
  private void init(AttributeSet attrs) {
    ...
    mShoppingText = TextUtils.isEmpty(typeArray.getString(R.styleable.ShoppingView_sv_text))
        ? DEFAULT_SHOPPING_TEXT : typeArray.getString(R.styleable.ShoppingView_sv_text);
    //展示文案大小
    int textSize = (int) typeArray.getDimension(R.styleable.ShoppingView_sv_text_size, sp2px(12));
    //背景色
    int bgColor = typeArray.getColor(R.styleable.ShoppingView_sv_bg_color,
        ContextCompat.getColor(getContext(), R.color.sv_blue));
    typeArray.recycle();
  ...
  private void drawBgMove(Canvas canvas) {
    ...
    canvas.drawRect(new RectF(mWidth + MAX_HEIGHT / 2, 0, MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT),
        mPaintBg);
    canvas.drawArc(new RectF(MAX_WIDTH - MAX_HEIGHT, 0, MAX_WIDTH, MAX_HEIGHT), 180, 270, false,
        mPaintBg);
  }
  ...
  private void drawAddBtn(Canvas canvas) {
    ...
    canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 4, MAX_WIDTH - MAX_HEIGHT / 2,
        MAX_HEIGHT / 4 * 3, mPaintText);
    canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 4, MAX_HEIGHT / 2,
        MAX_WIDTH - MAX_HEIGHT / 4, MAX_HEIGHT / 2, mPaintText);
  }
  ...
  private void drawMinusBtn(Canvas canvas, float angle) {
    ...
    canvas.drawCircle(mMinusBtnPosition, MAX_HEIGHT / 2, MAX_HEIGHT / 2 - MAX_HEIGHT / 20,
        mPaintMinus);
  ...
  private void drawNumText(Canvas canvas) {
    drawText(canvas, String.valueOf(mNum),
        mTextPosition - getTextWidth(mPaintNum, String.valueOf(mNum)) / 2f,
        MAX_HEIGHT / 2 + getTextHeight(String.valueOf(mNum), mPaintNum) / 2f, mPaintNum, mAngle);
  }
  ...
  @Override public boolean onTouchEvent(MotionEvent event) {
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (mState == STATE_NONE) {
          if (isPointInCircle(new PointF(event.getX(), event.getY()),
              new PointF(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 2), MAX_HEIGHT / 2)) {
            mNum++;
            startMoveAnim();
          ...
          if (isPointInCircle(new PointF(event.getX(), event.getY()),
            ...
            invalidate();
          } else if (isPointInCircle(new PointF(event.getX(), event.getY()),
              new PointF(MAX_HEIGHT / 2, MAX_HEIGHT / 2), MAX_HEIGHT / 2)) {
            if (mNum > 0) {
              mNum--;
              if (mShoppingClickListener != null) {
  ...
  private boolean isPointInCircle(PointF pointF, PointF circle, float radius) {
    return Math.pow((pointF.x - circle.x), 2) + Math.pow((pointF.y - circle.y), 2) <= Math.pow(
        radius, 2);
  }
